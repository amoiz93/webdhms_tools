...
variables = ['fcst_phy2m','fcst_surf']
...
for var in variables:
    for year in years:
        ...
        for month in months:
            ...
            inpath = os.path.join(base,var,str(year)+str(month).zfill(2),var+'.ctl')
            outpath = os.path.join(save,var)
            makedir(outpath)
            
            ga('run gradstonc.gs ' + inpath + ' ' + outpath)
