...
soil_code = np.unique(soil_unit, return_index = True, return_counts = True)
soil_index = soil_code[1][1:]
soil_code = {'Record':range(1, len(soil_code[0])),
             'VALUE':soil_code[0][1:].tolist(),
             'COUNT':soil_code[2][1:].tolist()}
...
alpha = arcpy.RasterToNumPyArray(alpha, nodata_to_value = -9999.0)
alpha = (alpha.flatten())[soil_index]

ks = arcpy.Raster(os.path.join(project, mapping, 'ks.tif'))
ks = arcpy.RasterToNumPyArray(ks, nodata_to_value = -9999.0)
ks = (ks.flatten())[soil_index]
ks = np.round(ks*10.0/24.0, 5)

thetar = arcpy.Raster(os.path.join(project, mapping, 'thetar.tif'))
thetar = arcpy.RasterToNumPyArray(thetar, nodata_to_value = -9999.0)
thetar = (thetar.flatten())[soil_index]

thetas = arcpy.Raster(os.path.join(project, mapping, 'thetas.tif'))
thetas = arcpy.RasterToNumPyArray(thetas, nodata_to_value = -9999.0)
thetas = (thetas.flatten())[soil_index]

watern = arcpy.Raster(os.path.join(project, mapping, 'watern.tif'))
watern = arcpy.RasterToNumPyArray(watern, nodata_to_value = -9999.0)
watern = (watern.flatten())[soil_index]

soil_code['alpha'] = alpha
soil_code['n'] = watern
soil_code['theta_r'] = thetar
soil_code['theta_s'] = thetas
soil_code['ks1'] = ks
soil_code['ks2'] = np.round(ks*0.1,5)
soil_code['ksg'] = np.round(ks*0.05,5)
soil_code['GWcs'] = 0.15
